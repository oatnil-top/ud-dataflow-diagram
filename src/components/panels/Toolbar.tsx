...
export default function Toolbar({ onOpenJsonImport, onOpenAICollab, onAIGenerate, embedMode, getViewportCenter }: ToolbarProps) {
  ...
  return (
    <div className="absolute top-2 left-2 sm:top-4 sm:left-4 z-10 flex flex-wrap gap-1.5 sm:gap-2 max-w-[calc(100vw-1rem)] sm:max-w-none">
      ...
      {/* AI Collaborate — copy the prompt to any chat, paste the answer back */}
      <button
        onClick={onOpenAICollab}
        className="flex items-center justify-center gap-1.5 sm:gap-2 bg-white hover:bg-violet-50 text-violet-700 px-2.5 sm:px-4 py-2 rounded-lg shadow-md border border-violet-200 transition-all text-xs sm:text-sm font-medium"
        title={t('resources.dataflow.menu.aiCollab')}
      >
        <BotMessageSquare size={16} />
        <span className="hidden sm:inline">{t('resources.dataflow.toolbar.aiCollab')}</span>
      </button>
